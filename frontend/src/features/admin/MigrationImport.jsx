    ...
    return (
        <div className="container" style={{ marginTop: 'var(--form-page-top, 2rem)' }}>
            <header className="dashboard-hero" style={{ marginBottom: 'var(--form-col-gap, 1.5rem)' }}>
                ...
                <p className="muted">
                    Upload the <strong>Migration Bundle ZIP</strong> generated by the old site
                    to populate the new DB with languages, parameters, questions, motivations
                    and answers already compiled.
                </p>
            </header>
            ...
            <div className="card" style={{ padding: 'var(--form-box-pad-lg, 1.5rem)', marginBottom: 'var(--form-col-gap, 1.5rem)', borderLeft: '4px solid #b91c1c' }}>
                ...
                <ul className="small" style={{ lineHeight: 1.7, marginBottom: 0 }}>
                    <li>With <code>wipe</code> enabled, <strong>all data tables are deleted</strong> (languages, parameters, questions, answers, examples, motivations, glossary, snapshots, change logs, taxonomy).</li>
                    <li>Existing users are NOT touched. The default admin is (re)created anyway with the credentials from the env vars <code>ADMIN_EMAIL</code>/<code>ADMIN_PASSWORD</code> (default: <code>admin@pcm.local</code>/<code>admin</code>).</li>
                    <li>All imported answers are saved as <strong>pending</strong> and the DAG runs for every language at the end of the import (Table A and the dashboard will be populated right away).</li>
                    <li>Meant to be used <strong>only once</strong>, at the first start-up of the new site. Afterwards, hide/remove this button.</li>
                </ul>
            </div>

            <div className="card" style={{ padding: 'var(--form-box-pad-lg, 1.5rem)', marginBottom: 'var(--form-col-gap, 1.5rem)' }}>
                <h3 style={{ marginTop: 0 }}>Expected bundle content</h3>
...
├── 00_languages.xlsx                       (registry + family/top/group)
...
├── 06_glossary.xlsx                        (Word, Description)        [optional]
├── 08_unsure_flags.xlsx                    (Language ID, Parameter ID) [optional]
└── data/
    ├── <Language1>.xlsx                    (Database_model sheet)
    └── ...`}
                </pre>
            </div>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            />
                            <span>
                                <strong>Full wipe</strong> before the import
                                <span className="muted small"> (recommended for the first migration)</span>
                            </span>
                        </label>
                    </div>
                    ...
                    {wipe && (
                        <div style={{ marginBottom: '1rem' }}>
                            <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '0.4rem' }}>
                                To confirm the wipe, type <code>WIPE</code> below:
                            </label>
                            <input
                                type="text"
                                value={confirmText}
    ...
    return (
        <div className="card" style={{ padding: 'var(--form-box-pad-lg, 1.5rem)', marginBottom: 'var(--form-col-gap, 1.5rem)' }}>
            ...
            <div className="small muted" style={{ marginTop: '0.75rem' }}>
                Long operation (it may take a few minutes). Do not close the page —
                you can reopen it at the same URL, but you would not see live progress.
            </div>
        </div>
    );
}
    ...
    return (
        <div className="card" style={{ padding: 'var(--form-box-pad-lg, 1.5rem)' }}>
            <h3 style={{ marginTop: 0 }}>
                {hasErrors || hasDagFails ? 'Migration completed with warnings' : 'Migration completed'}
            </h3>

            <div className="small muted" style={{ marginBottom: '1rem' }}>
                {report.wipe_performed && <span>✔ Wipe performed.&nbsp;</span>}
                {report.admin_email && <span>✔ Admin: <strong>{report.admin_email}</strong>.&nbsp;</span>}
                <span>✔ {report.languages_imported?.length || 0} languages imported.</span>
            </div>

            <h4 style={{ marginBottom: '0.5rem' }}>Summary by section</h4>
            ...
            {hasErrors && (
                <>
                    <h4 style={{ marginBottom: '0.5rem' }}>Errors ({report.errors.length})</h4>
                    <div style={{
            ...
            <div style={{ marginTop: 'var(--form-col-gap, 1.5rem)', display: 'flex', gap: '0.75rem' }}>
                <Link to="/languages" className="btn btn--primary">Go to the language list</Link>
