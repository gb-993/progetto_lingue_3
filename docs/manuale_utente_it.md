...
Ricorda: l'admin può forzare lo stato della lingua in qualsiasi momento e in qualsiasi direzione (vedi §6 del [documentation_it.md](docs/documentation_it.md) per il workflow completo).
...
Click su una riga → pagina di edit con:
...
- **Change history** — tabella di **tutte le modifiche** alle entità del sistema (parameter, question, motivation, language). Filtri per: tipo, ID, operazione (create/update/delete), source (manual / excel_import / system), utente, data, ricerca testuale. Ogni riga apre un drawer con il **diff prima/dopo**.
- **Answer changes** — stesso formato, focalizzato sulle modifiche alle Answer (ogni save di un blocco parametro genera entry qui).
- **Full backups (languages & parameters)** — lista degli **snapshot completi** raggruppati per timestamp:
    - i backup di tipo "languages" sono creati da [Languages → + Full Languages Backup](#6-la-pagina-languages);
    - i backup di tipo "parameters" sono creati da [Parameters → Backup current state](#8-solo-admin--parameters);
    - cliccando una riga si apre il dettaglio dello snapshot (lingua per lingua, parametro per parametro) con la possibilità di scaricare un Excel con tutti i dati.
- **Old questions archive** — domande archiviate (vedi §9). Per ogni domanda archiviata: snapshot del testo, motivations consentite all'epoca, e tutte le risposte storiche per ogni lingua.
...
Import strutturato da Excel. Cap 50 MB sul file. Il foglio deve seguire il formato del round-trip ([documentation_it.md](docs/documentation_it.md) §10.4).
